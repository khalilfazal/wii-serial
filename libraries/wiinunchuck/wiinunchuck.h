...
// is a more intelligent atan function. It returns the appropriate angle
// depending on which quadrant the vector <x,y> is in.
//
...
// Returns joystick rad values. It uses the ratio of calibrated
// x and y potentiometer readings to find the rad.
static inline double nunchuk_joyrad() {
    return sqrt(pow(nunchuk_cjoy_x(), 2) + pow(nunchuk_cjoy_y(), 2));
}
